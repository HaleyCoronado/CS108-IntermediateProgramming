...
public class Race extends JFrame {
	...
	private void prepareToRace() {
		...
		while (input != 's' && input != 'S') {
			switch (input) {
				case 't':
				case 'T':
					Racer tortoise = new Tortoise("Tortoise", START_LINE, yPos);
					racerList.add(tortoise);
					yPos += RACER_SPACE;
					break;
				case 'h':
				case 'H':
					Racer hare = new Hare( "Hare", START_LINE, yPos );
					racerList.add(hare);
					yPos += RACER_SPACE;
					break;
				case 'c':
				case 'C':
					Racer chicken = new Chicken( "Chicken", START_LINE, yPos );
					racerList.add(chicken);
					yPos += RACER_SPACE;
					break;
				case 'm':
				case 'M':
					Racer motorcycle = new Motorcycle( "Motorcycle", START_LINE, yPos );
					racerList.add(motorcycle);
					yPos += RACER_SPACE;
					break;
				default:
	...
	private char getRacer() {
		String input = JOptionPane.showInputDialog(this, "Enter a racer:"
				+ "\nt for Tortoise, h for hare, c for chicken, m for motorcycle" + "\nor s to start the race");
		if (input == null) {
